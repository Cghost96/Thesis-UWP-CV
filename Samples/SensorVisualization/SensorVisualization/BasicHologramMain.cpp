...
void BasicHologramMain::SaveAppState()
{
	...
	char accFile[512];
	std::snprintf(accFile, 512, "%s\\accelerometer.csv", charStr);
	...
	char gyroFile[512];
	std::snprintf(gyroFile, 512, "%s\\gyrometer.csv", charStr);
	...
	char magFile[512];
	std::snprintf(magFile, 512, "%s\\magnetometer.csv", charStr);
	std::ofstream magFileOut(magFile, std::ios::out);

	magFileOut << "#x, y, z, milliseconds, timeInMilliseconds\n";
